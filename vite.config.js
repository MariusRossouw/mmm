...
const commitHash = child.execSync('git rev-parse --short=7 --verify HEAD').toString();
const commitDate = child.execSync('git log -1 --format=%cI').toString().trimEnd();
			...
			ValidateEnv({
				VITE_CONFIG_URL: (key, value) => {
					if (process.env.VITE_FE && process.env.VITE_FE === 'local') {
						...
						if (value != null && value != undefined && value.includes('https://') === false) {
